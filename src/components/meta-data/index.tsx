...
import { ImageFragment, SeoDataFragment } from "@/graphql/fragments";
import { type FragmentType, getFragmentData } from "@/graphql/gql";
import { defaultSettings } from "@/providers/settings-provider";

export { SeoDataFragment } from "@/graphql/fragments";

export const MetaData: FC<{
  seo?: FragmentType<typeof SeoDataFragment> | null;
}> = ({ seo: seoProp }) => {
  const [env] = useState<EnvSettings>(SettingsUtil.create());
  const pathname = usePathname();
  const locale = useLocale();
  const seo = getFragmentData(SeoDataFragment, seoProp);
  const { title, description, image, robotsFollow, robotsIndex } = seo || {};
  const strapiImageUrl = getFragmentData(ImageFragment, image)?.url;
