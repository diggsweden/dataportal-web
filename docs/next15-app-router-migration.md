...
- [x] **Phase 4 (fortroendemodellen):** no dedicated pages needed — fortroendemodellen is a CMS container served through `[...containerSlug]`. The `dataportal_Digg_FoertroendemodellenBlock` block type is rendered by `BlockList` → `FortroendemodellenFrom` (client component). Removed `"fortroendemodellen"` from `PAGES_ROUTER_PREFIXES` in `proxy.ts` — the set is now empty, meaning all routes go through the App Router. `features/fortroendemodellen/fortroende-end-page/` and `fortroende-module-page/` are dead code (never imported); `features/pages/form-page/` is the legacy v1 form, superseded by `fortroendemodellen-v2`. All can be deleted in Phase 5 cleanup.
...
- [x] **Phase 5 (delete pages/):** deleted entire `pages/` directory — all routes now served by `app/[locale]/`. Removed `PAGES_ROUTER_PREFIXES` from `proxy.ts` (simplified rewrite logic). Removed `serverExternalPackages` for winston (only consumer was `pages/_error.tsx`). Deleted dead code: `utilities/logger.ts` (unused), `components/meta-data/` (replaced by `generateMetadata()`), `features/fortroendemodellen/` (dead — never imported), `resolvePage` + `populateSeo` from `utilities/app.ts` (only consumer was `pages/_app.tsx`). Removed stale Pages Router migration comment from `next.config.mjs`.
- [ ] **Phase 5:** audit `"use client"` boundaries, add `revalidateTag` webhook, update docs and run Lighthouse.
