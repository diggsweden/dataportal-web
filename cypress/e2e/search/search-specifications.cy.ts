...
describe("Search specifications", () => {
  ...
  it("Verify specifications search input", () => {
    ...
    cy.url().should("include", `/specifications?p=1&q=${SEARCH_INPUT}`);
  });

  it("Verify search result list has default 20 results", () => {
    cy.get("[data-test-id='search-result-list']")
      .should("exist")
      // Then wait for at least one li to exist to ensure the list is populated
      .find("li")
      .should("exist")
      // Finally, check the total count of li elements
      .then(() => {
        cy.get("[data-test-id='search-result-list'] li").should(
          "have.length",
          20,
        );
      });
