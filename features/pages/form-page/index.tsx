    ...
    data.forEach((item) => {
      ...
      if (item.__typename === "dataportal_Digg_FormRadio") {
        ...
        item.choices.forEach((choice, i) => {
          ...
          choice.ID = Number.parseInt(`${item.ID}${i}`, 10); //create a unique id by using the parent id and the index of the choice
        });
      }
      ...
      {formDataArray[0] && (
        ...
        >
          ...
          {formDataArray.map((data: FormTypes[], arrayIndex) => {
            const pageIndex = arrayIndex + 1; // page 0 is intro
            if (page !== pageIndex) {
              return null;
            }
            const stepKey = data
