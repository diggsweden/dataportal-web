...
export const keyword = (t: Translate) => {
  return {
    ...
    template:
      ...
      "</div>" +
      "<div>" +
      '{{#eachprop "dcat:keyword" limit=4 expandbutton="' +
...
export const getApiExploreUrl = (
  ...
  if (currentPath.startsWith("/dataservice")) {
    return `/datasets/${cid}_${entryid}/apiexplore/${apientryid}`;
  }

  return `${currentPath}/apiexplore/${apientryid}`;
};
...
export const hemvist = (t: Translate) => {
  return {
    ...
    run: function (node: any, data: any, items: any, entry: Entry) {
      ...
      if (
        resourceURI.indexOf("https://dataportal.se/") === 0 ||
        resourceURI.indexOf("https://www-sandbox.dataportal.se/") === 0
      ) {
        node.innerHTML =
          '<h3 class="!mt-none">' +
          linkTitle +
          ':</h3> <a class="hemvist mb-lg" href=' +
          resourceURI +
          ">" +
          resourceURI +
          "</a>";
